...
/>
<x-mixed-widget
title="معدل إكمال الفرصة"
description="تتبع تقدم جمع الاستثمارات لهذه الفرصة الاستثمارية"
button-text="{{ $investmentOpportunity->is_completed ? 'مكتملة' : ($investmentOpportunity->is_fundable ? 'قابلة للتمويل' : 'غير قابلة للتمويل') }}"
button-class="{{ $investmentOpportunity->is_completed ? 'btn-success' : ($investmentOpportunity->is_fundable ? 'btn-primary' : 'btn-secondary') }}"
button-action="#"
:chart-height="300"
chart-color="{{ $investmentOpportunity->completion_rate >= 100 ? 'success' : ($investmentOpportunity->completion_rate >= 75 ? 'warning' : 'danger') }}"
:legend-items="[
    ['color' => $investmentOpportunity->completion_rate >= 100 ? 'success' : ($investmentOpportunity->completion_rate >= 75 ? 'warning' : 'danger'), 'label' => 'مكتمل: ' . number_format($investmentOpportunity->completion_rate, 1) . '%'],
    ['color' => 'info', 'label' => 'المستهدف: $' . number_format($investmentOpportunity->target_amount, 0)],
    ['color' => 'primary', 'label' => 'المجمع: $' . number_format($investmentOpportunity->investments->sum('total_investment'), 0)]
]"
:menu-items="[
            ...
            <div class="card h-100">
                <div class="card-body d-flex flex-column justify-content-center">
                    <div class="text-center mb-5">
                        ...
                        <h4 class="fw-bold text-gray-800">معدل الإكمال</h4>
                    </div>

                    <div class="separator my-5"></div>

                    <div class="text-center">
                        <h5 class="fw-semibold text-gray-600 mb-3">حالة الفرصة</h5>
                        @if($investmentOpportunity->is_completed)
                            <span class="badge badge-success fs-6 px-4 py-2">مكتملة</span>
                        @elseif($investmentOpportunity->is_fundable)
                            <span class="badge badge-primary fs-6 px-4 py-2">قابلة للتمويل</span>
                        @else
                            <span class="badge badge-secondary fs-6 px-4 py-2">غير قابلة للتمويل</span>
                        @endif
                    </div>
                </div>
            </div>
...
{{--
            <x-mixed-widget
                title="حالة تسليم البضائع"
                description="{{ $investmentOpportunity->all_merchandise_delivered ? 'تم تسليم جميع البضائع بنجاح' : 'في انتظار تسليم البضائع' }}"
                button-text="{{ $investmentOpportunity->all_merchandise_delivered ? 'تم التسليم' : 'تأكيد التسليم' }}"
                button-class="{{ $investmentOpportunity->all_merchandise_delivered ? 'btn-success' : 'btn-warning' }}"
                button-action="#"
                :chart-height="250"
                chart-color="{{ $investmentOpportunity->all_merchandise_delivered ? 'success' : 'warning' }}"
                :legend-items="[
                    ['color' => $investmentOpportunity->all_merchandise_delivered ? 'success' : 'warning', 'label' => $investmentOpportunity->all_merchandise_delivered ? 'تم التسليم' : 'في الانتظار'],
                    ['color' => 'info', 'label' => 'إجمالي البضائع: ' . $investmentOpportunity->investments->count()]
                ]"
                :menu-items="[
                    'heading' => 'إدارة البضائع',
                    'items' => [
                        ['text' => 'عرض تفاصيل التسليم', 'url' => '#'],
                        ['text' => 'تحديث حالة التسليم', 'url' => '#'],
                        ['text' => 'طباعة إيصال التسليم', 'url' => '#'],
                        ['text' => 'إرسال إشعار للمستثمرين', 'url' => '#']
                    ]
                ]"
            /> --}}
        </div>
