...
class TaqnyatSms
{
     ...
     **/
    private function run($host, $path, $data = '', $requestType = "POST")
    {
        switch ($this->method) {
            case 'curl':
                $ch = curl_init();
            ...
            case 'fsockopen':
                ...
                $length = strlen($data);
                $fsockParameter = "$requestType $path HTTP/1.0\r\n";
            ...
            case 'file':
                $contextOptions['http'] = array('method' => $requestType, 'header' => 'Authorization: Bearer ' . $this->auth . "\r\n" . 'Content-type: application/x-www-form-urlencoded', 'content' => $data, 'max_redirects' => 0, 'protocol_version' => 1.0, 'timeout' => 10, 'ignore_errors' => true);
                $contextResource = stream_context_create($contextOptions);
                $arrayResult = file($host . $path, FILE_IGNORE_NEW_LINES, $contextResource);
                $this->result = $arrayResult[0];
                break;
            case 'file_get_contents':
                $contextOptions['http'] = array('method' => $requestType, 'header' => 'Authorization: Bearer ' . $this->auth . "\r\n" . 'Content-type: application/x-www-form-urlencoded', 'content' => $data, 'max_redirects' => 0, 'protocol_version' => 1.0, 'timeout' => 10, 'ignore_errors' => true);
                $contextResource = stream_context_create($contextOptions);
                $this->result = file_get_contents($host . $path, false, $contextResource);
                break;
            default:
